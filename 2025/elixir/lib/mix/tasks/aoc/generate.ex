defmodule Mix.Tasks.Aoc.Generate do
  ...
  def run(args) do
    ...
    if day not in 1..12, do: raise("invalid day #{day}")
  ...
  defp generate_solution(day_num) do
    day_mod = Module.concat([:Aoc25, :"Day#{day_num}"])
    if Code.ensure_loaded?(day_mod), do: raise("module #{day_mod} already exists")

    app_dir = File.cwd!()
    new_file_path = Path.join([app_dir, "lib", "aoc25", "day#{day_num}.ex"])
  ...
  defp generate_test(day_num) do
    test_mod = Module.concat([:Aoc25, :"Day#{day_num}Test"])
    if Code.ensure_loaded?(test_mod), do: raise("module #{test_mod} already exists")

    day_mod = Module.concat([:Aoc25, :"Day#{day_num}"])

    app_dir = File.cwd!()
    new_file_path = Path.join([app_dir, "test", "aoc25", "day#{day_num}_test.exs"])
